...
void menu(int s)
{
  int opcao = 0, qtdRespostas, count, confirmacao;  
  struct mensagens
  {
	  char usuario[10];
	  char texto[50];
  ...
  struct retorno
  {
	...
	switch(opcao)
	{
		...
		// Recebe a mensagem do servidor contendo a quantidade de respostas que serao enviadas pelo servidor 
		if(recv(s, &qtdRespostas, sizeof(qtdRespostas), 0) < 0)
		{
		...
		{
			for(count = 0; count <= qtdRespostas; count++);
			{
